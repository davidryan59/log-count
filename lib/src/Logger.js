...
const logText = (text) => {
  // Log some text to console, then return it
...
const log = (logger, inputData, level) => {
  ...
  if (logString.length > 0) return logText(`${logger.introText}${getLevelText(logger, level)}${logString}`);
  if (logObject) console.log(logObject); /* eslint-disable-line no-console */
  return '';
};
